...
function Hero({ onBookClick }) {
  ...
  // Глобальный scroll для parallax между секциями
  const { scrollY } = useScroll();

  // Parallax для Hero - остаётся на месте пока Services поднимается
  const heroOpacity = useTransform(scrollY, [0, 400, 800], [1, 1, 0]);
  const heroScale = useTransform(scrollY, [0, 800], [1, 1.1]);

  const springHeroScale = useSpring(heroScale, { stiffness: 100, damping: 30 });
  ...
  return (
    <section
      id="home"
      ref={containerRef}
      className="relative min-h-[100vh] flex items-start justify-center overflow-hidden"
      aria-label="Главный экран"
    >
      {/* Sticky контейнер для Hero */}
      <div className="sticky top-0 h-screen w-full flex items-center justify-center overflow-hidden z-20">
        {/* Parallax фон */}
        <motion.div
          className="absolute inset-0 z-0"
          style={{
            scale: isMobile ? 1 : springHeroScale,
            opacity: heroOpacity,
          }}
        >
        ...
        >
          ...
          >
            ...
            первый визит и{" "}
            <span className="font-medium text-text-dark">10%</span> на второй
            визит в студию
          </motion.p>
